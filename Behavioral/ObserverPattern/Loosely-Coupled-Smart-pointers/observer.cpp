...
private:
    ...
    {
        auto it = std::remove(mListObs.begin(), mListObs.end(), obs);
        mListObs.erase(it, mListObs.end());
    }
...
public:
    ...
    void removeObserver()
    {
        mCallbackRemove(shared_from_this());
    }
...
int main()
{
...

    {
        ...
        obsCall->removeObserver(); // Explicitly remove the observer before it goes out of scope
